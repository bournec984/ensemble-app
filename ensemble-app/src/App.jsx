...
const sb = {
  ...
  async del(table, query) {
    const h = query ? this.headers : {...this.headers, "Prefer": "return=minimal"};
...
async function saveMembers(names) {
  // Delete all existing members then reinsert
  await sb.del("members", "?id=gte.1");
...
function BulkImportModal({onClose,onAdd,isAdmin}) {
  ...
  if(step===STEPS.INPUT) return(
    <Modal title="Bulk Import" subtitle="Upload a filled template or load the SSO season directly." onClose={onClose} wide>
      ...
      <div style={{padding:"12px 14px",background:"#0F0E08",border:`1px solid ${G.border}`,borderRadius:2,marginBottom:14}}>
        <div style={{fontSize:12,color:G.creamDim,marginBottom:6,fontWeight:500}}>SSO 2026/27 already scraped</div>
      ...
      {error&&<div style={{fontSize:12,color:G.red,padding:"7px 12px",background:G.redFaint,borderRadius:2,border:`1px solid #4A1810`}}>{error}</div>}
...
function ViewToggle({isCalendar,onToggle}) {
  return(
    <div style={{display:"flex",border:`1px solid ${G.border}`,borderRadius:2,overflow:"hidden",flexShrink:0}}>
      {[["list","☰ List",false],["cal","📅 Calendar",true]].map(([k,label,isCal])=>(
        <button key={k} onClick={()=>onToggle(isCal)} style={{
          background:isCalendar===isCal?G.goldFaint:"none",
          border:"none",color:isCalendar===isCal?G.gold:G.textDim,
          padding:"5px 12px",cursor:"pointer",fontSize:11,fontFamily:"'Jost',sans-serif",
          letterSpacing:".08em",textTransform:"uppercase",transition:"all .15s"}}>{label}</button>
      ))}
    </div>
  );
}
...
// ── Calendar grid view ────────────────────────────────────────────────────────
function CalendarView({concerts,currentUser,onConcertClick}) {
  const today = new Date();
  // Find the month of the first upcoming concert, or current month
  const firstDate = concerts.find(c=>c.date&&new Date(c.date+"T12:00:00")>=today)?.date;
  const startDate = firstDate ? new Date(firstDate+"T12:00:00") : today;
  const [year,setYear]=useState(startDate.getFullYear());
  const [month,setMonth]=useState(startDate.getMonth()); // 0-indexed
  ...
  // Map concerts to day numbers for this month/year
  const concertsByDay = {};
  concerts.forEach(c=>{
    if(!c.date)return;
  ...
  return(
    <div className="fadeUp">
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
        <button onClick={prevMonth} style={{background:"none",border:"none",color:G.textDim,cursor:"pointer",fontSize:18,padding:"0 8px"}}>‹</button>
        <div style={{fontSize:16,fontFamily:"'Playfair Display',serif",color:G.cream,letterSpacing:.5}}>
          {MONTHS[month]} {year}
        </div>
        <button onClick={nextMonth} style={{background:"none",border:"none",color:G.textDim,cursor:"pointer",fontSize:18,padding:"0 8px"}}>›</button>
      </div>
      {/* Day headers */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,marginBottom:4}}>
        {DAYS.map(d=><div key={d} style={{textAlign:"center",fontSize:10,color:G.textFaint,
          fontFamily:"'Jost',sans-serif",letterSpacing:1.5,textTransform:"uppercase",padding:"4px 0"}}>{d}</div>)}
      </div>
      {/* Calendar grid */}
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2,overflow:"hidden"}}>
        {cells.map((day,i)=>{
          if(!day) return <div key={`e${i}`}/>;
          const dayC=concertsByDay[day]||[];
          const isToday=new Date().getDate()===day&&new Date().getMonth()===month&&new Date().getFullYear()===year;
          const myGoing=dayC.some(c=>c.interest?.[currentUser]);
          const purchased=dayC.some(c=>c.purchases?._buyer);
          return(
            <div key={day} onClick={()=>dayC.length&&onConcertClick(dayC[0])} style={{
              minHeight:64,padding:"5px 6px",borderRadius:2,cursor:dayC.length?"pointer":"default",
              background:dayC.length?(purchased?G.greenFaint:myGoing?G.goldFaint:G.card):G.bg,
              border:`1px solid ${dayC.length?(purchased?"#1E4020":myGoing?G.goldDim:G.borderLight):G.border}`,
              transition:"all .15s",overflow:"hidden",minWidth:0,
            }}
            onMouseEnter={e=>{if(dayC.length)e.currentTarget.style.filter="brightness(1.15)";}}
            onMouseLeave={e=>e.currentTarget.style.filter="none"}>
              <div style={{fontSize:11,fontFamily:"'Jost',sans-serif",
                color:isToday?G.gold:dayC.length?G.cream:G.textDim,
                fontWeight:isToday?600:400,marginBottom:2}}>{day}</div>
              {dayC.slice(0,2).map((c,ci)=>{
                // Smart short title: use soloists if present, else first meaningful word chunk
                const soloist = c.soloists?.[0]?.name?.split(" ").slice(-1)[0]||"";
                const shortTitle = soloist
                  ? soloist + (c.soloists.length>1?` +${c.soloists.length-1}`:"")
                  : c.title.replace(/^SSO\s+(Chamber|Pops|Organ Series|Concerts for Children|Lunchtime|National Day|Christmas|Babies)[\s:–-]*/i,"").slice(0,18);
                const timeStr = c.time?.replace(":","h")||"";
                return(
                  <div key={ci} style={{
                    fontSize:9,color:purchased?G.green:myGoing?G.gold:G.creamDim,
                    lineHeight:1.4,marginBottom:1,fontFamily:"'Jost',sans-serif",
                    overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis",
                    maxWidth:"100%",display:"block",
                  }} title={`${c.time} ${c.title}`}>
                    <span style={{opacity:.7}}>{timeStr}</span>{timeStr?" ":""}{shortTitle}
                  </div>
                );
              })}
              {dayC.length>2&&<div style={{fontSize:9,color:G.textDim,fontFamily:"'Jost',sans-serif",marginTop:1}}>+{dayC.length-2}</div>}
            </div>
          );
        })}
      </div>
      {/* Legend */}
      <div style={{display:"flex",gap:16,marginTop:12,fontSize:11,color:G.textDim,fontFamily:"'Jost',sans-serif"}}>
        <span><span style={{color:G.gold}}>●</span> Interested</span>
        <span><span style={{color:G.green}}>●</span> Purchased</span>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const[calView,setCalView]=useState({concerts:false,mytickets:false});
    ...
    <main style={{maxWidth:900,margin:"0 auto",padding:"30px 20px"}}>
      ...
      {tab==="concerts"&&<>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:22,gap:12,flexWrap:"wrap"}}>
          <div style={{fontSize:13,color:G.textDim,paddingLeft:10,borderLeft:`2px solid ${G.goldFaint}`,lineHeight:1.6,flex:1}}>
            See what's coming up and tap Going? on anything that catches your eye. Once someone buys tickets, you'll see what you owe and can add the concert to your calendar.
          </div>
          <ViewToggle isCalendar={calView.concerts} onToggle={v=>setCalView(p=>({...p,concerts:v}))}/>
        </div>
        {approved.length===0&&<EmptyState label="No concerts yet. Use Bulk Import or Add Concert above."/>}
        {calView.concerts
          ?<CalendarView concerts={approved} currentUser={currentUser} onConcertClick={c=>setExpandedId(c.id)}/>
          :approved.map(c=>(
        ...
        {calView.concerts&&expandedId&&(()=>{
          const c=approved.find(x=>x.id===expandedId);
          if(!c)return null;
          return <div style={{marginTop:16}}>
            <ConcertCard concert={c} currentUser={currentUser} users={members}
              expanded={true}
      ...
      {tab==="mytickets"&&<>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          <div style={{fontSize:13,color:G.textDim}}>Concerts you're attending or interested in.</div>
          <ViewToggle isCalendar={calView.mytickets} onToggle={v=>setCalView(p=>({...p,mytickets:v}))}/>
        </div>
        {myTickets.length===0&&<EmptyState label="No tickets yet. Mark your interest on the Concerts tab."/>}
        {calView.mytickets
          ?<CalendarView concerts={myTickets} currentUser={currentUser} onConcertClick={()=>{}}/>
          :myTickets.map(c=>{
